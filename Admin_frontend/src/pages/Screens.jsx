  ...
  <PermissionGate routeName="SCREEN_VIEW">
    <div className="space-y-6 w-full overflow-x-hidden">
      ...
      <div className="bg-slate-900 rounded-lg border border-slate-800 shadow-xl overflow-x-auto">
        <table className="min-w-[900px] w-full text-left text-sm text-slate-400">
                    ...
                    <div className="flex flex-wrap gap-1 max-w-[380px]">
                      {Array.isArray(s.routes) &&
                        s.routes.map((r, i) => (
                          <span
                            key={i}
                            className="px-2 py-0.5 bg-slate-800 rounded text-xs text-blue-300 font-mono whitespace-nowrap"
                          >
                            {r}
                          </span>
                        ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-lg p-5 sm:p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {formData.routes.map((route, idx) => (
                    <span
                      key={idx}
                      className="flex items-center gap-1 bg-blue-900/30 border border-blue-900 text-blue-300 px-2 py-1 rounded text-xs font-mono"
                    >
                      {route}
                      <button
                        type="button"
