      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-900 rounded-lg border border-slate-800 overflow-hidden shadow-xl">
          <table className="w-full text-left text-sm text-slate-400">
            ...
            <tbody>
              {materials.length === 0 ? (
                <tr><td colSpan="5" className="p-6 text-center text-slate-500">No materials found.</td></tr>
              ) : materials.map((m) => (
                <tr key={m._id} className="border-t border-slate-800 hover:bg-slate-800/50">
                  <td className="p-4 font-medium text-white">{m.name}</td>
                  <td className="p-4">{m.unit}</td>
                  <td className="p-4">
                    <span className={m.current_quantity < 10 ? "text-red-400 font-bold" : "text-slate-300"}>
                      {m.current_quantity}
                    </span>
                  </td>
                  <td className="p-4">
                    <span className={`text-xs px-2 py-0.5 rounded ${m.isActive ? "bg-green-900 text-green-300" : "bg-red-900 text-red-300"}`}>
                      {m.isActive ? "Active" : "Inactive"}
                    </span>
                  </td>
                  <td className="p-4 text-right space-x-3">
                    <PermissionGate routeName="RAW_MATERIAL_UPDATE">
                      <button onClick={() => openEditModal(m)} className="text-blue-400 hover:text-blue-300">Edit</button>
                    </PermissionGate>
                    <PermissionGate routeName="RAW_MATERIAL_DELETE">
                      <button onClick={() => handleDelete(m._id)} className="text-red-400 hover:text-red-300">Delete</button>
                    </PermissionGate>
                  </td>
                </tr>
              ))}
